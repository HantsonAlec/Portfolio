...
class Qagent:
    def __init__(self, nr_actions, alpha, lr_decay, gamma, exploration_rate, decay):
    ...
    def update_qtable(self, state, new_state, reward):
        ...
        self.r = random.random()
        if self.r <= self.exploration_rate:
            self.next_q_value = self.q_table[self.new_state, random.sample(
                self.possibleActions, 1)[0]]
        else:
            self.next_q_value = self.q_table[self.new_state, np.argmax(
                self.q_table[self.new_state, :])]
...
max_t = 200
q_agent = Qagent(nr_actions, alpha, lr_decay, gamma, exploration_rate, decay)
